...
| Firebase backend | `OPTIVUS_BACKEND=firebase` plus matching `OPTIVUS_FIREBASE_PROJECT_ID` for live/release | Initializes Firebase and selects implemented Firebase Auth/Profile/Onboarding/settings/upload-review plus Routine template/occurrence/transaction/Habit Systems repositories | No silent fallback in conditional providers, and Firestore rules have a local emulator suite, but deployment, restart, and cross-device behavior still require verification. Most non-Routine post-onboarding repositories remain fake or unavailable. |
