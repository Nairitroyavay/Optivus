...
2026-07-23. The product blueprint describes product behavior and ownership;
...
| Routine | Routine items | Local | In-memory fake repository in fake mode | Explicit Firebase mode selects `FirestoreRoutineRepository` at `users/{uid}/routineItems/{itemId}` | Verified owner-scoped Firestore behind one Routine controller | `lib/features/routine/routine_state.dart`; `lib/repositories/routine_repository.dart`; `ROUTINE_DATA_CONTRACT.md` | Canonical codec/selection exist and no Firebase flow writes the mock list; deployed Firebase, restart/device, cross-device, and emulator evidence are missing. |
| Routine | Routine editing | Local | Canonical Routine notifier with fake or Firestore per-document repository writes | Firebase mode uses owner-scoped create/update/delete and no whole-list replacement | Owner-scoped Firestore CRUD through the canonical Routine controller | `lib/features/routine/routine_state.dart`; `lib/repositories/routine_repository.dart` | Durable code exists, but full Phase 4 CRUD UX/acceptance and remote persistence verification remain. |
| Routine | Routine history | Local | In-memory occurrence repository in fake mode | Firebase mode uses `users/{uid}/routineHistory/{occurrenceId}` with stable dated IDs | Verified owner-scoped Routine occurrence records | `lib/repositories/routine_history_repository.dart`; `ROUTINE_DATA_CONTRACT.md` | Daily state is separated from templates and Firestore-capable; deployed restart/cross-device and emulator evidence are missing. |
...
| Routine | Onboarding-to-Routine projection | Local | Fake mode has an atomic in-memory equivalent | Firebase completion transaction writes absent deterministic templates plus create-only `onboarding-initial-v1` receipt; restore checks the receipt and loads canonical data | Verified idempotent projection into one durable Routine repository | `FirestoreOnboardingRepository`; `RoutineOnboardingProjection`; `OnboardingFrontendHydrationService` | Edit/delete/no-op/atomic-failure tests pass locally; deployment, emulator, physical restart, and cross-device acceptance are missing. |
...
| Routine | One canonical controller; fake mode is in-memory and Firebase mode is Firestore-capable for templates/occurrences/projection | Verified owner-scoped Firestore plus later habit-system and complete UX acceptance |
...
| Firebase backend | `OPTIVUS_BACKEND=firebase` plus matching `OPTIVUS_FIREBASE_PROJECT_ID` for live/release | Initializes Firebase and selects implemented Firebase Auth/Profile/Onboarding/settings/upload-review plus Routine template/occurrence repositories | No silent fallback in conditional providers, but deployment, rules, restart, emulator, and cross-device behavior still require verification. Most non-Routine post-onboarding repositories remain fake. |
...
| --- | --- | --- |
| R2 Upload Worker | `/health`, `/v1/uploads/sign`, `/v1/uploads/complete`, `/v1/uploads/delete`; dev and separate staging-template `UPLOAD_BUCKET` bindings | Pending remote verification; 2026-07-23 typecheck, 14 request tests, staging type generation, and dry run passed. Exact account/name/project/bucket/origins, deploy version, URL, and smoke evidence are missing. |
| Routine Import Worker | `/health`, `/v1/routine-import/extract`, `/v1/routine-import/classes`, `/v1/routine-import/work`, `/v1/routine-import/eating-photo`; dev and separate staging-template `UPLOAD_BUCKET` bindings | Pending remote verification; 2026-07-23 typecheck, 13 request tests, staging type generation, and dry run passed. Exact targets, deploy version, URL, and smoke evidence are missing. |
| Nutrition Worker | `/health`, `/v1/eating/generate-routine` | Pending remote verification; 2026-07-23 typecheck, 12 request tests, staging type generation, and dry run passed. Exact targets, deploy version, URL, and smoke evidence are missing. |
| Skin Care Worker | `/health`, `/v1/skin-care/products/analyze`, `/v1/skin-care/routine/generate`; dev and separate staging-template `UPLOAD_BUCKET` bindings | Pending remote verification; 2026-07-23 typecheck, 52 tests, staging type generation, and dry run passed. Exact targets, deploy version, URL, and authenticated smoke evidence are missing. |
| Coach Worker | `/health`, `/v1/coach/reply` | Pending remote verification; 2026-07-23 typecheck, 11 request tests, staging type generation, and dry run passed. Exact targets/deploy/smoke are missing, and the active Coach tab does not use the client. |
| Firebase | Auth plus selected Firestore repository implementations and rules | Pending for an exact named staging environment and any future approved production environment; live/release startup is fail-closed, but no remote/device evidence exists. |
