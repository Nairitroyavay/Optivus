...
void main() {
  ...
  group('Group I Adversarial & Stress Tests', () {
    ...
    group('Adversarial Test 1: 200% Font Scale & Long Strings Overflow Protection', () {
      testWidgets('OnboardingChip withstands 200% font scale and 250-char string in narrow container', (tester) async {
        const longString = 'Extremely_Long_Adversarial_Chip_Label_String_1234567890_That_Exceeds_Normal_Container_Bounds_And_Tests_Text_Scaler_Clamping_And_Fitted_Box_Overflow_Protection_In_Optivus_UI_Component_Library_Verification_Test_Suite_Padding_Border_Check';
        ...
        expect(tester.takeException(), isNull, reason: 'OnboardingChip threw an exception under 200% font scale with long string');
        expect(find.byType(OnboardingChip), findsOneWidget);
      });

      testWidgets('OnboardingActionPill withstands 200% font scale and long string in narrow container', (tester) async {
        const longString = 'Adversarial_Action_Pill_Label_Under_200_Percent_Text_Scale_Factor_Extreme_Length_Test_Case';
        ...
        await tester.pump(const Duration(milliseconds: 300));
        expect(tester.takeException(), isNull, reason: 'OnboardingActionPill overflowed or threw exception');
        expect(find.byType(OnboardingActionPill), findsOneWidget);
      });

      testWidgets('OnboardingChoiceTile withstands 200% font scale with massive title and subtitle', (tester) async {
        const longTitle = 'Adversarial Choice Tile Title That Is Unusually Extended Beyond Standard Component Width Requirements';
        const longSubtitle = 'Adversarial Subtitle With Multiple Words Explaining Details Under Extremely High Display Scale Settings';
        ...
        await tester.pump(const Duration(milliseconds: 300));
        expect(tester.takeException(), isNull, reason: 'OnboardingChoiceTile threw layout overflow exception');
        expect(find.byType(OnboardingChoiceTile), findsOneWidget);
      });

      testWidgets('Multiple chip/pill flex row renders under 200% font scale without render flex overflow', (tester) async {
        await tester.pumpWidget(
          MaterialApp(
            home: MediaQuery(
              ...
              child: Scaffold(
                body: SizedBox(
                  ...
                  child: Wrap(
                    ...
                    runSpacing: 8,
                    children: const [
                      OnboardingChip(label: 'Very Long Option Pill 1', selected: true),
                      OnboardingChip(label: 'Very Long Option Pill 2', selected: false),
                      OnboardingActionPill(label: 'Action Button 1', selected: true),
                      OnboardingActionPill(label: 'Action Button 2', selected: false),
                    ],
                  ),
                ),
              ),
    ...
    // ──────────────────────────────────────────────────────────────────────────
    group('Adversarial Test 2: Rapid Toast Error Triggers & FIFO Queue Management', () {
      test('ToastQueueNotifier processes rapid triggers in exact FIFO sequence', () {
      ...
      test('ToastQueueNotifier deduplicates rapid identical toast requests', () {
        final container = ProviderContainer();
        addTearDown(container.dispose);
        ...
        notifier.showToast('Duplicate Error Message'); // Should be ignored (matches current)
        notifier.showToast('Unique Error Message');
        notifier.showToast('Duplicate Error Message'); // Should be ignored (matches queued)
      ...
      test('ToastQueueNotifier dismissCurrent advances FIFO queue sequentially to empty', () async {
        final container = ProviderContainer();
        addTearDown(container.dispose);
        ...
        expect(container.read(toastQueueProvider).current?.message, equals('Toast A'));

        // Dismiss Toast A -> Toast B becomes active after 150ms delay
        notifier.dismissCurrent();
        await Future.delayed(const Duration(milliseconds: 200));
        expect(container.read(toastQueueProvider).current?.message, equals('Toast B'));
        expect(container.read(toastQueueProvider).queue.length, equals(1));

        // Dismiss Toast B -> Toast C becomes active after 150ms delay
        notifier.dismissCurrent();
        await Future.delayed(const Duration(milliseconds: 200));
        expect(container.read(toastQueueProvider).current?.message, equals('Toast C'));
      ...
      test('ToastQueueNotifier clearAll immediately purges active and queued toasts', () {
        final container = ProviderContainer();
        addTearDown(container.dispose);
      ...
      testWidgets('Toast error banner renders correctly in OnboardingStepShell', (tester) async {
        await tester.pumpWidget(
          MaterialApp(
    ...
    // ──────────────────────────────────────────────────────────────────────────
    group('Adversarial Test 3: Back Swipe / Pop Gesture on Dirty vs Clean Draft Steps', () {
      testWidgets('Pop gesture on DIRTY step triggers Unsaved Changes dialog', (tester) async {
        ...
        final container = ProviderContainer(
          overrides: [
            mockOnboardingProvider.overrideWith((_) {
              final notifier = MockOnboardingNotifier()..loadSeedData(dirtyDraft);
        ...
        expect(find.text('You have unsaved changes on this step. Do you want to save or discard before going back?'), findsOneWidget);
      ...
      testWidgets('Pop gesture on CLEAN step navigates back directly without Unsaved Changes dialog', (tester) async {
        ...
        final container = ProviderContainer(
          overrides: [
            mockOnboardingProvider.overrideWith((_) {
              final notifier = MockOnboardingNotifier()..loadSeedData(cleanDraft);
    ...
    // ──────────────────────────────────────────────────────────────────────────
    group('Adversarial Test 4: Dark Mode Theme Brightness Switching & Tokens', () {
      testWidgets('Onboarding widgets adapt theme brightness switching dynamically', (tester) async {
        final themeNotifier = ValueNotifier<ThemeData>(OptivusTheme.lightTheme);
              ...
              return MaterialApp(
                home: Theme(
                  ...
                  child: Scaffold(
                    body: Column(
                      children: const [
                        OnboardingGlassCard(child: Text('Glass Card Content')),
                        OnboardingChip(label: 'Dark Mode Chip', selected: false),
                        OnboardingActionPill(label: 'Dark Mode Pill', selected: true),
        ...
        BuildContext contextLight = tester.element(find.text('Glass Card Content'));
        ...
        BuildContext contextDark = tester.element(find.text('Glass Card Content'));
        expect(Theme.of(contextDark).brightness, equals(Brightness.dark));
        expect(tester.takeException(), isNull, reason: 'Switching theme brightness to dark mode caused rendering exception');

        // Check text colors and tokens in dark theme
        final displayLargeColor = Theme.of(contextDark).textTheme.displayLarge?.color;
        ...
        BuildContext contextLightAgain = tester.element(find.text('Glass Card Content'));
        expect(Theme.of(contextLightAgain).brightness, equals(Brightness.light));
        expect(tester.takeException(), isNull);
      });

      test('OptivusTheme darkTheme defines valid contrast dark color palette', () {
        final darkTheme = OptivusTheme.darkTheme;
        expect(darkTheme.brightness, equals(Brightness.dark));
        expect(darkTheme.colorScheme.surface, equals(const Color(0xFF1E202A)));
      });
    });
  });
}
