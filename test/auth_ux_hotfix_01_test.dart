...
void main() {
  ...
  group('AUTH-UX-HOTFIX-01: Logo Animation & Header Transitions', () {
    testWidgets('TEST A: SignupScreen normal state has expanded logo and header', (
      tester,
    ) async {
    ...
    testWidgets('TEST B: SignupScreen with nonzero viewInsets.bottom enters compact layout', (
      tester,
    ) async {
      await _setDeviceView(tester, width: 360, height: 800);
    ...
    testWidgets('TEST C: SignupScreen returning viewInsets.bottom to zero restores expanded layout', (
      tester,
    ) async {
      await _setDeviceView(tester, width: 360, height: 800);
    ...
    testWidgets('TEST D: LoginScreen transitions smoothly between expanded and compact states', (
      tester,
    ) async {
      await _setDeviceView(tester, width: 360, height: 800);
    ...
    testWidgets('TEST E: Switching focus between fields while keyboard is open does not restart/expand logo', (
      tester,
    ) async {
      await _setDeviceView(tester, width: 360, height: 800);
      await tester.pumpWidget(
    ...
    testWidgets('TEST F: Password visibility toggle while keyboard visible preserves compact state', (
      tester,
    ) async {
      await _setDeviceView(tester, width: 360, height: 800);
      await tester.pumpWidget(
    ...
    testWidgets('Multi-frame progressive IME animation smoothly interpolates without snaps', (
      tester,
    ) async {
      await _setDeviceView(tester, width: 360, height: 800);
      ...
      expect(intermediateWidth, greaterThanOrEqualTo(AuthLayout.compactLogoSize));
  ...
  group('AUTH-UX-HOTFIX-01: Background & Scaffolding Architecture', () {
    testWidgets('TEST K & L: Scaffold has non-transparent Auth background painting full window', (
      tester,
    ) async {
      await _setDeviceView(tester, width: 360, height: 800);
      await tester.pumpWidget(_authScreen(const SignupScreen()));
      ...
      expect(loginScaffold.backgroundColor, equals(AuthLayout.authBackgroundColor));
      expect(loginScaffold.backgroundColor, isNot(equals(Colors.transparent)));
      expect(tester.takeException(), isNull);
    });
  ...
  group('AUTH-UX-HOTFIX-01: Responsive Viewports & Overflow Guards', () {
    ...
    for (final (name, width, height) in viewports) {
      testWidgets('TEST G & I: SignupScreen keyboard open on $name has no overflow', (
        tester,
      ) async {
      ...
      testWidgets('TEST H & I: LoginScreen keyboard open on $name has no overflow', (
        tester,
      ) async {
        await _setDeviceView(tester, width: width, height: height);
        await tester.pumpWidget(
    ...
    testWidgets('TEST J: Normal layout remains fully usable after keyboard dismiss', (
      tester,
    ) async {
      await _setDeviceView(tester, width: 360, height: 800);
