...
void main() {
  ...
  group('Group J Edge Case 1: PII Redaction on Combined & Complex Patterns', () {
    test('redacts combined PII elements (email, phone, bearer token, auth token, user name) in single text', () {
      const input =
    ...
    test('handles empty strings and null userName gracefully', () {
      ...
      expect(PiiRedactor.redact('No PII here', userName: null), equals('No PII here'));
      expect(PiiRedactor.redact('No PII here', userName: ''), equals('No PII here'));
      expect(PiiRedactor.redact('No PII here', userName: ' a '), equals('No PII here')); // Short trimmed name < 2 chars
    });
    ...
    test('documents international vs US phone number regex coverage', () {
      ...
      expect(PiiRedactor.redact('+1-555-123-4567'), contains('[REDACTED_PHONE]'));
      expect(PiiRedactor.redact('(555) 987-6543'), contains('[REDACTED_PHONE]'));
      ...
      expect(isIntlRedacted, isFalse, reason: 'PiiRedactor regex assumes 3-digit area code (\\d{3}), leaving 2-digit international area codes unredacted.');
    });
  });

  group('Group J Edge Case 2: Debouncer Flush Behavior during Rapid Navigation', () {
    test('rapid consecutive executions cancel previous tasks and only run the latest', () async {
    ...
    test('flush() on pending action prevents duplicate execution when delay elapses', () async {
    ...
    test('cancel() prevents execution even if flush() is called after', () async {
      final debouncer = Debouncer(delay: const Duration(milliseconds: 100));
      bool executed = false;
  ...
  group('Group J Edge Case 3: Wake Lock Release Safety on Sub-Action Failure', () {
    test('runWithWakeLock releases lock when syncTask throws an async error late in execution', () async {
      ...
      expect(
        () async => runWithWakeLock<void>(
          ...
          syncTask: () async {
            await Future<void>.delayed(const Duration(milliseconds: 10));
            throw const FormatException('Malformed response in background sync');
          },
        ),
    ...
    test('OnboardingCompletionJobService releases wake lock when profile repo throws error in stage 5', () async {
  ...
  group('Group J Edge Case 4: Platform Channel Safe Fallback Defaults', () {
    test('safePlatformCall catches generic Exception/Error and triggers onError callback', () async {
    ...
    test('NotificationIntentService returns fallback when channel returns unexpected types or throws exception', () async {
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMethodCallHandler(
            const MethodChannel('com.nairitroy.optivus/notification_intent'),
            (methodCall) async {
              throw PlatformException(code: 'UNAVAILABLE', message: 'Service down');
            },
          );
