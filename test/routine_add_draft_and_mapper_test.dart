...
void main() {
  group('AddRoutineDraft and Mapper Matrix', () {
    ...
    test('Maps all 6 types to corresponding RoutineItem', () {
      ...
      final checkInDraft =
          AddRoutineDraft.initial(
            ...
            checkInState: const AddRoutineCheckInState(checkInType: 'Smoking'),
          );
    ...
    test('Zero state leakage on type switching', () {
      ...
      draft = draft
          .switchType(AddRoutineType.tracker)
          .copyWith(
            trackerState: const AddRoutineTrackerState(
    ...
    test('bestTime roundtrip for Flexible Tasks', () {
      for (final bestTime in const [
        ...
        final draft = AddRoutineDraft.initial(
          initialType: AddRoutineType.flexible,
        ).copyWith(title: 'Task for $bestTime', bestTime: bestTime);
      ...
      final oneTimeDraft =
          AddRoutineDraft.initial(
            ...
            repeatDays: [
              ...
              2,
              3,
            ], // should be ignored when scheduleMode is once
          );
      ...
      final weeklyDraft =
          AddRoutineDraft.initial(
            ...
            date: DateTime(
              ...
              9,
              25,
            ), // should be ignored when scheduleMode is weekly
            repeatDays: [1, 3, 5],
          );
  ...
  group('AddRoutineValidator Invariants', () {
    test('Validates title is non-empty and non-whitespace', () {
      final emptyTitleDraft = AddRoutineDraft.initial(
        initialType: AddRoutineType.flexible,
      ).copyWith(title: '   ');
      ...
      final validTitleDraft = AddRoutineDraft.initial(
        initialType: AddRoutineType.flexible,
      ).copyWith(title: 'Morning Jog');
    ...
    test('Validates duration must be positive', () {
      final zeroDurationDraft = AddRoutineDraft.initial(
        initialType: AddRoutineType.flexible,
      ).copyWith(title: 'Quick Stretch', durationMinutes: 0);
    ...
    test('Validates repeatDays for weekly routine', () {
      ...
      expect(
        invalidResult,
        contains('Repeat days must be between 1 (Monday) and 7 (Sunday).'),
      );
    });

    test('Overnight Sleep block is valid', () {
      final sleepDraft =
          AddRoutineDraft.initial(initialType: AddRoutineType.fixed).copyWith(
            title: 'Sleep',
  ...
  group('AddRoutineSheet UI Responsiveness', () {
    ...
    testWidgets('Renders at 320dp width and 2.5x text scale without overflow', (
      tester,
    ) async {
        ...
        UncontrolledProviderScope(
          ...
          child: MaterialApp(
            home: MediaQuery(
              ...
              child: const Scaffold(body: AddRoutineSheet()),
            ),
          ),
        ),
      );
