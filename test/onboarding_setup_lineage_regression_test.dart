...
void main() {
  ...
  group('Physical restore defect regression: completion_run_input_mismatch', () {
    test(
      ...
      () {
        ...
        final draft = _validDraftAtStep(uid, 3, setupGeneration: 1, revision: 3);
  ...
  group('Full 12 Lineage States Matrix', () {
    // 1. Fresh user
    test('1. Fresh user with no durable onboarding data returns ReconstructionFresh', () {
      final profile = UserProfile.empty(uid: uid).copyWith(currentSetupGeneration: 0);
      final result = classifyServerReconstruction(
        ownerUid: uid,
        profile: profile,
        draft: null,
    ...
    test('2. Incomplete draft in same generation returns ReconstructionIncomplete', () {
      final profile = UserProfile.empty(uid: uid).copyWith(currentSetupGeneration: 0);
    ...
    test('3. Older generation run is superseded and draft resumes normally', () {
    ...
    test('5. Same-lineage final draft with active run returns ReconstructionFinishing', () {
      final profile = UserProfile.empty(uid: uid).copyWith(currentSetupGeneration: 1);
    ...
    test('7. Same-lineage run mismatch returns Recovery(completion_run_input_mismatch)', () {
      final profile = UserProfile.empty(uid: uid).copyWith(currentSetupGeneration: 1);
      final draft = _validDraftAtStep(uid, 3, setupGeneration: 1, revision: 2);
    ...
    test('8. Same-lineage fatal failure returns Recovery(completion_run_fatal)', () {
      final profile = UserProfile.empty(uid: uid).copyWith(currentSetupGeneration: 1);
      final draft = _finalDraft(uid, setupGeneration: 1);
      final runId = stableOnboardingRunId(
        ownerUid: uid,
    ...
    test('9. Future generation run returns Recovery(setup_lineage_corrupt)', () {
      final profile = UserProfile.empty(uid: uid).copyWith(currentSetupGeneration: 1);
    ...
    test('10. Mismatched draft generation returns Recovery(setup_lineage_corrupt)', () {
      final profile = UserProfile.empty(uid: uid).copyWith(currentSetupGeneration: 2);
      final draft = _validDraftAtStep(uid, 3, setupGeneration: 1); // draft is behind profile
      final result = classifyServerReconstruction(
        ownerUid: uid,
        profile: profile,
        draft: draft,
    ...
    test('12. Legacy pointer without setupGeneration behaves as generation 0', () {
      // If profile is generation 0, it is evaluated as same lineage
      final profileGen0 = UserProfile.empty(uid: uid).copyWith(currentSetupGeneration: 0);
      ...
      final profileGen1 = UserProfile.empty(uid: uid).copyWith(currentSetupGeneration: 1);
  ...
  group('Step-14 Completion Writer Lineage Fencing', () {
    test('Late completion writer from old generation is rejected by proof evaluation', () {
      final profile = UserProfile.empty(uid: uid).copyWith(
      ...
      expect(proof.disposition, CompletionTerminalizationDisposition.notEligible);
      expect(proof.reason, CompletionTerminalizationReason.invalidCompletionBundle);
    });
  });

  group('OnboardingSetupResetCoordinator', () {
    test('Atomic reset updates profile, draft, and marks in-memory run superseded', () async {
      ...
      final initialProfile = UserProfile.empty(uid: uid).copyWith(
