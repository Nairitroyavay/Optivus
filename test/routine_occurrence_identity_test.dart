...
void main() {
  group('Routine Day Occurrence Identity & Dual-Instance Rendering', () {
    ...
    test(
      'Overnight recurring template produces two entries on the same day with distinct instanceIds',
      () {
        ...
        expect(
          continuationEntry.instanceId,
          isNot(equals(scheduledEntry.instanceId)),
        );
        expect(
          continuationEntry.instanceId,
          'c:${overnightSleepTemplate.id}:2026-09-15',
        );
        expect(
          scheduledEntry.instanceId,
          's:${overnightSleepTemplate.id}:2026-09-16',
        );
    ...
    test(
      'Moved-in occurrence alongside native template produces distinct instanceIds',
      () {
        ...
        final nativeEntry = entries.firstWhere(
          (e) => e.kind == RoutineDayEntryKind.scheduled,
        );
        final movedEntry = entries.firstWhere(
          (e) => e.kind == RoutineDayEntryKind.movedIn,
        );
    ...
    test(
      'Moved overnight occurrence projects target start and one source-identical continuation',
      () {
        final occurrence = RoutineOccurrenceRecord(
          id: 'occ_sleep_sep15',
          ownerUid: 'user_1',
          routineItemId: overnightSleepTemplate.id,
          occurrenceDateKey: '2026-09-15',
          status: RoutineStatus.moved,
          source: 'routine',
          action: 'reschedule',
          operationKey: 'move_sleep',
        ...
        final sourceStart = RoutineOccurrenceProjector.entriesForDay(
          [overnightSleepTemplate],
          [occurrence],
          DateTime(2026, 9, 15),
        );
        final sourceContinuation = RoutineOccurrenceProjector.entriesForDay(
          [overnightSleepTemplate],
          [occurrence],
          DateTime(2026, 9, 16),
        );
        final targetStart = RoutineOccurrenceProjector.entriesForDay(
          [overnightSleepTemplate],
          [occurrence],
          DateTime(2026, 9, 20),
        );
        final targetContinuation = RoutineOccurrenceProjector.entriesForDay(
          [overnightSleepTemplate],
          [occurrence],
          DateTime(2026, 9, 21),
        );
        ...
        expect(
          sourceStart.where((e) => e.occurrenceDateKey == '2026-09-15'),
          isEmpty,
        );
        expect(
          sourceContinuation.where((e) => e.occurrenceDateKey == '2026-09-15'),
          isEmpty,
        );
        final movedStart = targetStart.singleWhere(
          (e) => e.occurrenceId == occurrence.id,
        );
        final movedContinuation = targetContinuation.singleWhere(
          (e) => e.occurrenceId == occurrence.id,
        );
        expect(movedStart.kind, RoutineDayEntryKind.movedIn);
        expect(movedStart.item.startMinute, 23 * 60);
        expect(movedStart.item.endMinute, 1440);
        expect(movedContinuation.kind, RoutineDayEntryKind.continuation);
        expect(movedContinuation.item.startMinute, 0);
        expect(movedContinuation.item.endMinute, 6 * 60 + 30);
        expect(movedContinuation.occurrenceDateKey, '2026-09-15');
        expect(movedContinuation.templateId, movedStart.templateId);
        final seed = RoutineMoveSeedResolver.resolve(
          actionContext: RoutineActionContext.fromDayEntry(movedContinuation),
          visibleItem: movedContinuation.item,
          templates: [overnightSleepTemplate],
          occurrences: [occurrence],
        );
        expect(seed.startMinute, 23 * 60);
        expect(seed.durationMinutes, 450);
      },
    );

    testWidgets(
      'RoutinePreparedTimelineLayout prepares duplicate template entries without assertion or map overwrite',
      (tester) async {
    ...
    testWidgets(
      'RoutineTimelineViewport renders both instances of overnight template with NO red screen or duplicate key exception',
      (tester) async {
        final selectedDay = DateTime(2026, 9, 16);
        final entries = RoutineOccurrenceProjector.entriesForDay(
          [overnightSleepTemplate],
        ...
        expect(
          find.byKey(ValueKey('routine-timeline-card-$continuationId')),
          findsOneWidget,
        );
        expect(
          find.byKey(ValueKey('routine-timeline-card-$scheduledId')),
          findsOneWidget,
        );
    ...
    test(
      'InstanceId derivation is strictly deterministic and stable across multiple invocations',
      () {
