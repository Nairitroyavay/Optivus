...
void main() {
  group('Routine Timeline Viewport Scroll Hardening', () {
    testWidgets(
      'Scroll to current time handles early frame without throwing null check exception',
      (tester) async {
    ...
    testWidgets(
      'Transitioning from non-today to isToday does not trigger scroll exception',
      (tester) async {
        final now = DateTime.now();
        final currentMinute = now.hour * 60 + now.minute;
