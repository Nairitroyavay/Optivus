...
void main() {
  ...
  group('K: Routine Cards Have Zero Conflict Styling or Badges', () {
    ...
    testWidgets('FlexibleTaskCard renders without Conflict badge', (tester) async {
      final item = RoutineItem(
        id: 'flex_1',
        title: 'Review PRs',
        startMinute: 660,
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: FlexibleTaskCard(item: item),
          ),
        ),
      );
    ...
    testWidgets('TrackerTaskCard renders without Conflict badge', (tester) async {
      final item = RoutineItem(
        id: 'track_1',
        title: 'Water Log',
        startMinute: 720,
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: TrackerTaskCard(item: item),
          ),
        ),
      );
  ...
  group('M: AddRoutineSheet Saves Overlapping Items Without Conflict Box', () {
    testWidgets('Save button is enabled and no conflict box appears when overlapping existing item', (tester) async {
      ...
      // Fill in title
      await tester.enterText(find.byType(TextField).first, 'New Overlapping Meeting');
      await tester.pumpAndSettle();
      ...
      final saveBtnFinder =
          find.widgetWithText(ElevatedButton, 'Save at this time');
    ...
    testWidgets('Button says Move task and allows overlap', (tester) async {
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            routineNotifierProvider.overrideWith(
              (ref) => _FakeNotifier(ref, initialItems: [itemToMove, existingItem]),
            ),
          ],
          child: MaterialApp(
            home: Scaffold(
              body: Consumer(
                builder: (context, ref, _) {
                  return ElevatedButton(
                    onPressed: () => showRoutineMoveSheet(context, ref, itemToMove),
  ...
  group('O: AIAssistantSheet Has No Conflict Suggestions', () {
    testWidgets('Does not show Fix conflict suggestions or chips', (tester) async {
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            routineNotifierProvider.overrideWith(
  ...
  group('P: WeekPlannerSheet Has No Conflict Chips', () {
    testWidgets('Does not display conflict counts or warning colors', (tester) async {
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            routineNotifierProvider.overrideWith(
  ...
  group('Q: RoutineTab Zero Conflict UX', () {
    testWidgets('No conflict banner, no conflict filter, no resolver toggle', (tester) async {
      final item1 = RoutineItem(
        id: 'i1',
        title: 'Class 1',
        startMinute: 540,
      ...
      final allFiltered = TimelineUtils.filterItems([item1, item2], 'conflicts');
      expect(allFiltered.length, 2);
    });
  });

  group('R: Card Tap Directly Opens Detail Sheet', () {
    testWidgets('Tapping overlapping card opens detail sheet directly without resolver', (tester) async {
  ...
  group('A, B, C, F, G, H, I: Real RoutineNotifier Overlap Writes (No Fake)', () {
    test('A & F: Add overlap with simultaneous hard blocks succeeds', () async {
    ...
    test('I: Closely spaced and overlapping meals succeed without 120-min error', () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
  ...
  group('D: Drag into Overlap Accepts Target Time', () {
    testWidgets('Drag drop to overlapping position saves without error snackbar', (tester) async {
      tester.view.physicalSize = const Size(800, 1400);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
  ...
  group('E: Multiple Simultaneous Items Materialization & Collision Layout', () {
    test('At least 3-4 simultaneous overlapping activities remain materialized with no conflicts', () {
      final container = ProviderContainer();
      addTearDown(container.dispose);
  ...
  group('P: Routine Settings Automation Section Has No Conflict Resolver Switch', () {
    testWidgets('Settings screen contains AI and Notification toggles but NO Conflict Resolver', (tester) async {
      tester.view.physicalSize = const Size(800, 1400);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
  ...
  group('S: Legacy Conflict Metadata Neutralization', () {
    test('Legacy hasConflict, conflictMessage, and allowedConflicts do not leak into materialized items', () {
      final container = ProviderContainer();
      addTearDown(container.dispose);
    ...
    testWidgets('Legacy conflict metadata in state does not produce conflict banner or badges in RoutineTab', (tester) async {
      final legacyItem = RoutineItem(
        id: 'legacy_ui_1',
