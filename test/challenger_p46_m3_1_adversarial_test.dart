  ...
  group('Challenger P46 M3.1: Onboarding Recovery State Transitions & Draft Handling', () {
    ...
    test(
      ...
      () async {
        ...
        expect(savedDraft?.onboardingCompleted, isFalse);
      },
    );

    test(
      'executeRecoveryAction when maxAttemptsReached is true forces onboarding incomplete',
      () async {
        final fakeAuthRepo = FakeAuthRepository();
        final fakeOnboardingRepo = FakeOnboardingRepository();
        final container = ProviderContainer(
        ...
        final retryController = container.read(recoveryRetryControllerProvider.notifier);
        for (var i = 0; i < 5; i++) {
  ...
  group('Challenger P46 M3.1: Auth Sign-Out & Account Switching Isolation', () {
    ...
    test(
      ...
      () async {
        ...
        expect(container.read(authProvider).user?.uid, equals('account-b-switch'));
    ...
    test(
      ...
      () async {
        ...
        expect(authState.status, equals(AuthFlowStatus.signedInOnboardingComplete));
      },
    );
    ...
    test(
      ...
      () async {
        ...
        final finalState = container.read(authProvider);
        expect(finalState.status, equals(AuthFlowStatus.signedInOnboardingIncomplete));
        expect(finalState.user?.uid, equals('rapid-user-9'));
      },
    );
  });

  group('Challenger P46 M3.1: Firestore Contract Boundary Limits & Serializer Robustness', () {
    test('UserProfile.fromMap handles nulls, optional fields, and unexpected keys', () {
    ...
    test('UserModel.fromMap handles null/missing optional fields and unexpected keys', () {
      final input = <String, dynamic>{
        'id': 'um-456',
        'email': 'um@test.com',
        'displayName': null,
    ...
    test('OnboardingDraft.fromMap handles missing steps, legacy step arrays, and corrupted sub-maps', () {
      final input = <String, dynamic>{
        'uid': 'draft-789',
        'currentStep': 99,
        'stepCompleted': [true, false],
        'lifeRole': 'corrupted_string_instead_of_map',
        'bodyBasics': <String, dynamic>{
          'heightCm': 180,
          'weightKg': null,
        },
    ...
    test('RoutineItem.fromMap handles string/int timestamps, nulls, corrupted enum strings', () {
      final input = <String, dynamic>{
        'id': 'item-001',
        'userId': 'user-001',
        'title': 'Test Item',
    ...
    test('HabitSystemRecord.fromMap handles Timestamp, int dates, and invalid enums', () {
      final input = <String, dynamic>{
        'systemId': 'sys-001',
        'ownerUid': 'user-123',
      ...
      expect(record.updatedAt, equals(DateTime.fromMillisecondsSinceEpoch(1772409600000, isUtc: true)));
    });

    test('OnboardingCompletionJob.fromMap handles null lists, unexpected keys, and invalid enums', () {
      final input = <String, dynamic>{
        'jobId': 'job-999',
        'uid': 'user-999',
    ...
    test('UserProfileSettings.fromMap handles empty and unexpected inputs', () {
      final input = <String, dynamic>{
        'name': 'Alice',
        'username': null,
    ...
    test('RegionSettings.fromMap handles missing/invalid enums and defaults safely', () {
      final input = <String, dynamic>{
        'userId': 'user-reg-1',
        'countryCode': 'US',
      ...
      expect(settings.measurementSystem, equals(MeasurementSystem.imperial));
    });
    ...
    test('RoutineOccurrenceRecord.fromMap handles nulls and invalid status', () {
      final input = <String, dynamic>{
        'id': 'occ-123',
        'ownerUid': 'u-occ-1',
