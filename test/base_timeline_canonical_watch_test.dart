...
void main() {
  group('Base Timeline Canonical Watch and Monotonicity Tests', () {
    group('Snapshot Filtering Policy (shouldPublishBaseTimelineSnapshot)', () {
      ...
      test(
        'Committed snapshot is accepted (hasPendingWrites: false, exists: true)',
        () {
          final result = shouldPublishBaseTimelineSnapshot(
            exists: true,
      ...
      test(
        'Committed cached snapshot is accepted (hasPendingWrites: false, isFromCache: true)',
        () {
          final result = shouldPublishBaseTimelineSnapshot(
            exists: true,
            hasPendingWrites: false,
    ...
    group('Remote Setup Monotonicity Policy (shouldPublishRemoteSetup)', () {
      ...
      test('Older revision is rejected', () {
        ...
        expect(shouldPublishRemoteSetup(current: base, remote: older), isFalse);
      });

      test('Higher revision is accepted', () {
        final newer = base.copyWith(revision: 4);
        expect(shouldPublishRemoteSetup(current: base, remote: newer), isTrue);
      });
      ...
      test(
        'Equal revision with divergent data is rejected (in-memory committed state wins)',
        () {
    ...
    group('BaseTimelineSetupNotifier Live Stream Integration', () {
      ...
      test(
        'Monotonic revision enforcement: older revision ignored, newer revision accepted',
        () async {
          ...
          repo.emitRemote(
            initial.copyWith(revision: 2, mealPlanningGoal: 'stale-rev-2'),
          );
          ...
          repo.emitRemote(
            initial.copyWith(revision: 4, mealPlanningGoal: 'newer-rev-4'),
          );
          await Future<void>.delayed(Duration.zero);
      ...
      test(
        'Schema version migration safety: rev 3 schema v2 in memory accepts rev 3 schema v3 from remote',
        () async {
      ...
      test(
        'Equal revision divergent state preserves in-memory committed setup',
        () async {
          ...
          expect(
            notifier.state.requireValue.mealPlanningGoal,
            'committed-in-memory',
          );
      ...
      test(
        'Generation switch: old watch subscription cancelled and does not leak events',
        () async {
          ...
          // Emit on repo1
          repo1.emitRemote(
            setup1.copyWith(revision: 2, mealPlanningGoal: 'post-dispose'),
          );
          await Future<void>.delayed(Duration.zero);
      ...
      test(
        'Malformed remote data (e.g. wrong owner) handled gracefully as error without crashing',
        () async {
