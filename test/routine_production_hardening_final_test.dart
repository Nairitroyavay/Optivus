...
class ControllableTransactionRepository
  ...
  Future<void> commitWrite({
    ...
    if (deleteOccurrenceId != null && failAfterDeleteTransport) {
      ...
      throw Exception('transport_error: network timeout waiting for ACK (DELETE)');
    }
  }
}
...
void main() {
  group('Routine Production Hardening: 40 Verification Gates', () {
    ...
    // ========================================================================
    test('Gates 1-2: Queued action rebases against rolled-back state when predecessor fails', () async {
      ...
      final startFuture = notifier.startRoutineItem(item.id, occurrenceDate: date);
      ...
      expect(state.occurrences.firstWhere((o) => o.routineItemId == item.id).status, RoutineStatus.active);
      ...
      expect(state.queuedOccurrenceActionsById[occId]!.first.action, RoutineOccurrenceAction.skip);
      ...
      state = container.read(routineNotifierProvider);
      final finalOcc = state.occurrences.firstWhere((o) => o.routineItemId == item.id);
    ...
    // ========================================================================
    test('Gates 3-4: Queued action rebases against committed state when predecessor succeeds', () async {
      final notifier = container.read(routineNotifierProvider.notifier);
      ...
      expect(container.read(routineNotifierProvider).pendingOccurrenceIds.contains(occId), isTrue);

      // Queue Complete while Start is pending
      final completeFuture = notifier.completeRoutineItem(item.id, occurrenceDate: date);
    ...
    // ========================================================================
    test('Gate 5: Start on Moved occurrence sets previousStatus: moved and preserves move schedule', () async {
      final notifier = container.read(routineNotifierProvider.notifier);
      ...
      final startRes = await notifier.startRoutineItem(item.id, occurrenceDate: sourceDate);
      expect(startRes.outcome, RoutineWriteOutcome.saved);

      final state = container.read(routineNotifierProvider);
      final occ = state.occurrences.firstWhere((o) => o.routineItemId == item.id);
      ...
      final startEvent = events.validEvents.firstWhere((e) => e.eventType == RoutineEventType.started);
      expect(startEvent.itemSnapshot['movedToDateKey'], routineLocalDateKey(targetDate));
    ...
    // ========================================================================
    test('Gate 6: Stop on started Moved occurrence restores Moved record with timer cleared', () async {
      final notifier = container.read(routineNotifierProvider.notifier);
      ...
      final stopRes = await notifier.undoOccurrenceAction(item.id, occurrenceDate: sourceDate);
      expect(stopRes.outcome, RoutineWriteOutcome.saved);

      var state = container.read(routineNotifierProvider);
      var occ = state.occurrences.firstWhere((o) => o.routineItemId == item.id);
      ...
      final undoMoveRes = await notifier.undoOccurrenceAction(item.id, occurrenceDate: sourceDate);
      expect(undoMoveRes.outcome, RoutineWriteOutcome.saved);

      state = container.read(routineNotifierProvider);
      expect(state.occurrences.where((o) => o.routineItemId == item.id), isEmpty);
    });

    // ========================================================================
    // GATE 7: Skip on Moved Occurrence & Undo Skip
    // ========================================================================
    test('Gate 7: Skip on Moved occurrence records moved fields; Undo Skip restores Moved status', () async {
      final notifier = container.read(routineNotifierProvider.notifier);
      ...
      final skipRes = await notifier.markSkipped(item.id, occurrenceDate: sourceDate);
      ...
      final undoRes = await notifier.undoOccurrenceAction(item.id, occurrenceDate: sourceDate);
    ...
    // ========================================================================
    test('Gate 8: Direct Done is undoable; Done after Start is terminal', () async {
      final notifier = container.read(routineNotifierProvider.notifier);
      ...
      var state = container.read(routineNotifierProvider);
      var occDirect = state.occurrences.firstWhere((o) => o.routineItemId == itemDirect.id);
      ...
      final undoDirectRes = await notifier.undoOccurrenceAction(itemDirect.id, occurrenceDate: date);
      expect(undoDirectRes.outcome, RoutineWriteOutcome.saved);
      state = container.read(routineNotifierProvider);
      expect(state.occurrences.where((o) => o.routineItemId == itemDirect.id), isEmpty);
      ...
      await notifier.completeRoutineItem(itemStarted.id, occurrenceDate: date);

      state = container.read(routineNotifierProvider);
      var occStarted = state.occurrences.firstWhere((o) => o.routineItemId == itemStarted.id);
    ...
    // ========================================================================
    test('Gates 9-10: Stacked active generic card actionFooterHeight calculates exact 2 rows', () {
      ...
      expect(
        genericPlan.rows[0].first.type,
        RoutineCardActionType.countdown,
      );
    ...
    // ========================================================================
    test('Gates 11-12: RoutineCardFactory.layoutFingerprint changes with undoToPlannedAllowed', () {
    ...
    // ========================================================================
    test('Gates 13-14: loadForOwner reconciles and purges stale tracker links via replaceAll', () async {
      final notifier = container.read(routineNotifierProvider.notifier);
      final trackerLinksNotifier = container.read(trackerSessionLinksProvider.notifier);
    ...
    // ========================================================================
    test('Gate 15: openTrackerSession aborts safely without fabricating fake session ID', () async {
      final notifier = container.read(routineNotifierProvider.notifier);
      const uid = 'user_g15';
      await notifier.loadForOwner(uid);
      ...
      notifier.openTrackerSession(trackerItem.id, occurrenceDate: DateTime(2026, 9, 20));
    ...
    // ========================================================================
    test('Gates 16-17: Account switching isolates state and resets tracker links', () async {
      final notifier = container.read(routineNotifierProvider.notifier);
      ...
      expect(container.read(routineNotifierProvider).activeTrackerLaunchIntent?.sessionId, 'sess_A');
      expect(container.read(trackerSessionLinksProvider).length, 1);
    ...
    // ========================================================================
    test('Gates 18-19: Delete ambiguity recovery verifies event existence via fetchEventById', () async {
      final notifier = container.read(routineNotifierProvider.notifier);
      const uid = 'user_g18';
      await notifier.loadForOwner(uid);
      ...
      var state = container.read(routineNotifierProvider);
      expect(state.occurrences.where((o) => o.routineItemId == item.id).first.status, RoutineStatus.completed);

      // 2. Undo Complete with network drop after delete commits on remote
      txRepo.failAfterDeleteTransport = true;
      final undoRes = await notifier.undoOccurrenceAction(item.id, occurrenceDate: date);
    ...
    // ========================================================================
    test('Gates 20-21: Set ambiguity recovery verifies remote write via fetchEventById', () async {
      final notifier = container.read(routineNotifierProvider.notifier);
      const uid = 'user_g20';
      await notifier.loadForOwner(uid);
      ...
      final startRes = await notifier.startRoutineItem(item.id, occurrenceDate: date);
    ...
    // ========================================================================
    test('Gates 36-40: Complete multi-step lifecycle: Plan -> Move -> Start -> Stop -> Skip -> Undo -> Undo', () async {
      final notifier = container.read(routineNotifierProvider.notifier);
      const uid = 'user_g36';
      await notifier.loadForOwner(uid);
      ...
      final undoSkipRes = await notifier.undoOccurrenceAction(item.id, occurrenceDate: sourceDate);
    ...
    // ========================================================================
    test('Gates 22-25: Add Routine draft ID is stable and findWeeklyFreeSlot finds common slot', () {
      final notifier = container.read(routineNotifierProvider.notifier);
    ...
    // ========================================================================
    test('Gates 26-30: Money task enforces savings invariant; alreadySaved satisfies requirement', () async {
      final notifier = container.read(routineNotifierProvider.notifier);
      const uid = 'user_g26';
      await notifier.loadForOwner(uid);
      ...
      final directRes = await notifier.completeRoutineItem(moneyItem.id, occurrenceDate: date);
      ...
      final state = container.read(routineNotifierProvider);
      final occ = state.occurrences.firstWhere((o) => o.routineItemId == moneyItem.id);
      expect(occ.status, RoutineStatus.completed);
      expect(occ.source, 'money');

      // Subsequent complete is safely treated as a no-op
      final repeatRes = await notifier.completeRoutineItem(moneyItem.id, occurrenceDate: date);
    ...
    // ========================================================================
    test('Gates 31-35: Overnight routine spanning midnight anchors to source occurrence date', () async {
      final notifier = container.read(routineNotifierProvider.notifier);
      const uid = 'user_g31';
      await notifier.loadForOwner(uid);
      ...
      final state = container.read(routineNotifierProvider);
      final occ = state.occurrences.firstWhere((o) => o.routineItemId == overnightItem.id);
