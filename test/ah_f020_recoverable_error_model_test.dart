...
void main() {
  group('AH-F020: RecoverableError Shared Model', () {
    ...
    test('B. Severity is independent from blocking', () {
      ...
      expect(
        blockingWarning.severity,
        equals(RecoverableErrorSeverity.warning),
      );
    ...
    test('C. retrySafe is independent from retryAction', () {
      ...
      expect(
        errorWithActionButNotSafe.retryAction,
      ...
      expect(
        errorSafeWithoutAction.retryAction,
  ...
  group('AH-F020: Mandatory Invariant Tests (A through P)', () {
    test(
      'Test A: Verify Email too-many-requests maps to authentication (NOT aiQuota)',
      () {
        ...
        expect(
          error.diagnosticCode,
          equals(DiagnosticCodes.verifyEmailRateLimited),
        );
        expect(error.retrySafe, isFalse);
        expect(error.publicMessage, contains('Too many verification attempts'));
      },
    );

    test(
      'Test B: Same Upload category in two contexts (required -> blocking, optional -> non-blocking)',
      () {
        ...
        expect(
          requiredUploadError.category,
          equals(RecoverableErrorCategory.upload),
        );
    ...
    test(
      'Test C: Preview network failure is non-blocking even when category is network',
      () {
        final previewError = UploadErrorMapper.previewUnavailable(
          isNetworkFailure: true,
        );
        expect(previewError.category, equals(RecoverableErrorCategory.network));
        expect(previewError.isBlocking, isFalse);
        expect(
          previewError.diagnosticCode,
    ...
    test(
      'Test D: Completion contradiction maps to recoveryRequired (never completionRetry merely because StateError)',
      () {
        ...
        expect(
          mapped.category,
          isNot(equals(RecoverableErrorCategory.completionRetry)),
        );
    ...
    test(
      'Test E: Completion transient verification failure maps to completionRetry with resumeCompletion',
      () {
        ...
        final mapped = CompletionErrorMapper.map(job: job, stage: job.stage);

        expect(
          mapped.category,
          equals(RecoverableErrorCategory.completionRetry),
        );
        expect(
          mapped.retryAction,
          equals(RecoverableRetryAction.resumeCompletion),
        );
        expect(mapped.retrySafe, isTrue);
        expect(
          mapped.diagnosticCode,
          equals(DiagnosticCodes.completionVerifyFailed),
        );
      },
    );
    ...
    test('Test F: Missing photo maps to validation', () {
      ...
      expect(
        missingError.category,
        equals(RecoverableErrorCategory.validation),
      );
      expect(missingError.retrySafe, isFalse);
      expect(
        missingError.diagnosticCode,
    ...
    test(
      'Test G: Corrupt selected photo maps to upload with chooseAnother and retrySafe=false',
      () {
        ...
        expect(
          corruptImageError.category,
          equals(RecoverableErrorCategory.upload),
        );
        expect(
          corruptImageError.retryAction,
          equals(RecoverableRetryAction.chooseAnother),
        );
        expect(corruptImageError.retrySafe, isFalse);
        expect(
          corruptImageError.diagnosticCode,
    ...
    test(
      'Test H: Metadata write failure after binary upload maps to cloudPersistence',
      () {
    ...
    test('Test I: Stale/late error protection', () {
      ...
      expect(
        ...
        isNull,
        reason:
            'Stale error from superseded Op A must not overwrite current state.',
      );
    });
    ...
    testWidgets(
      'Test K: Raw error firewall blocks secret tokens from UI widgets',
      (tester) async {
        const rawSecret = 'RAW_INTERNAL_SECRET_TOKEN_12345';
        final rawException = Exception(
          'FirebaseException(500): $rawSecret at /internal/auth/token',
        );
        ...
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(body: RecoverableErrorBanner(error: authError)),
          ),
        );
        ...
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(body: RecoverableErrorCard(error: authError)),
          ),
        );
        await tester.pumpAndSettle();
        ...
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(body: RecoverableErrorInline(error: authError)),
          ),
        );
        await tester.pumpAndSettle();
    ...
    test(
      'Test M: Two scoped errors coexist (stale preview warning + current blocking completion retry)',
      () {
        ...
        const completionError = RecoverableError(
          ...
          publicMessage:
              'Your setup is saved, but final preparation didn’t finish.',
          severity: RecoverableErrorSeverity.error,
          isBlocking: true,
    ...
    test('Test N: Mock/fake backend error mapping', () {
      ...
      expect(
        fakeAiError.diagnosticCode,
        equals(DiagnosticCodes.aiRoutineQuota),
      );
      expect(fakeAiError.isBlocking, isTrue);
    });

    test(
      'Test O: Source-firewall audit handles unmapped raw exceptions safely',
      () {
    ...
    test(
      'Test P: RecoveryRequired + restartRecovery action is non-destructive',
      () {
  ...
  group(
    'AH-F020: AI, Upload, Persistence, Conflict, Reconstruction Mappers',
    () {
      test('AI timeout mapper', () {
        ...
        expect(
          mapped.diagnosticCode,
          equals(DiagnosticCodes.aiNutritionTimeout),
        );
        expect(mapped.retrySafe, isTrue);
        expect(
          mapped.retryAction,
          equals(RecoverableRetryAction.retryGeneration),
        );
      });
      ...
      test('Upload binary failure mapper', () {
        ...
        expect(
          binaryFailure.diagnosticCode,
          equals(DiagnosticCodes.uploadBinaryFailed),
        );
        expect(binaryFailure.retrySafe, isTrue);
        expect(
          binaryFailure.retryAction,
          equals(RecoverableRetryAction.retryUpload),
        );
      });
      ...
      test('Permission permanent denial mapper', () {
        ...
        expect(
          permError.diagnosticCode,
          equals(DiagnosticCodes.permissionPermanentlyDenied),
        );
        expect(
          permError.retryAction,
          equals(RecoverableRetryAction.openSettings),
        );
  ...
  group('AH-F020: Step 14 Failure Matrix', () {
    ...
    test('Matrix C: Recovery required', () {
      final mapped = CompletionErrorMapper.map(isContradiction: true);
      expect(
        mapped.category,
        equals(RecoverableErrorCategory.recoveryRequired),
      );
  ...
  group('AH-F020: Comprehensive Raw Error Firewall & Category Matrix', () {
    ...
    test(
      'Firewall A: Injected RAW_FIREBASE_SECRET_4711 is blocked across all mappers',
      () {
        final ex = Exception(
          '$rawFirebaseSecret: internal token leak at /auth/db',
        );
        ...
        expect(
          authMapped.category,
          equals(RecoverableErrorCategory.authentication),
        );

        final friendlyAuth = authMapped.publicMessage;
        ...
        final persistMapped = PersistenceErrorMapper.mapStepSyncFailure(
          error: ex,
        );
    ...
    test(
      'Firewall B: Injected RAW_WORKER_BODY_9922 is blocked across all mappers',
      () {
        final workerEx = Exception(
          '$rawWorkerBody: 500 error from cloudflare worker body json',
        );
    ...
    test('Firewall C: Injected RAW_PLATFORM_DETAIL_8833 is blocked', () {
      final platformEx = Exception(
        '$rawPlatformDetail: native iOS camera permission crash in AVFoundation',
      );
    ...
    test(
      'Firewall D: Injected RAW_AI_RESPONSE_7711 is blocked from public messages',
      () {
    ...
    testWidgets('Firewall E: Production UI widgets never render raw secrets', (
      tester,
    ) async {
      ...
      for (final secret in injectedSecrets) {
        final err = AuthErrorMapper.map(
          Exception('$secret: database connection failed'),
        );

        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(body: RecoverableErrorBanner(error: err)),
          ),
        );
        await tester.pumpAndSettle();
        ...
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(body: RecoverableErrorCard(error: err)),
          ),
        );
        await tester.pumpAndSettle();
        expect(find.textContaining(secret), findsNothing);
        ...
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(body: RecoverableErrorInline(error: err)),
          ),
        );
        await tester.pumpAndSettle();
        expect(find.textContaining(secret), findsNothing);
    ...
    test(
      'Validation category is distinct from Network (no check connection copy)',
      () {
        final missingField = TimelineErrorMapper.missingRequiredBlocks(
          'Please select your time blocks.',
        );
        expect(
        ...
        expect(
          missingField.publicMessage.toLowerCase(),
    ...
    test('Network category is distinct from 429 quota and auth expired', () {
      ...
      expect(
        quotaMapped.category,
        isNot(equals(RecoverableErrorCategory.network)),
      );
    ...
    test('AI timeout has retrySafe=true and retryAction=retryGeneration', () {
      ...
      expect(
        mapped.publicMessage.toLowerCase(),
        isNot(contains('check your connection')),
      );
    });

    test(
      'Upload invalid image has retrySafe=false and retryAction=chooseAnother',
      () {
        ...
        expect(
          invalidImg.retryAction,
          equals(RecoverableRetryAction.chooseAnother),
        );
        expect(
          invalidImg.diagnosticCode,
          equals(DiagnosticCodes.uploadInvalidImage),
        );
    ...
    test(
      'Matrix O: Accepted AH-F015 Keep Both overlap must not surface as an error',
      () {
    ...
    test(
      'Matrix AD: AH-F019 footer CTA contract integrates with RecoverableError retry actions',
      () {
        final error = PersistenceErrorMapper.mapStepSyncFailure();
    ...
    test(
      'Matrix AE: AH-F016 AI generation lifecycle fencing and retry semantics',
      () {
    ...
    test(
      'Matrix AF: AH-F017 upload error mapper preserves two-phase replacement model',
      () {
    ...
    test(
      'Matrix AG: AH-F018 timeline conflict mapper correctly identifies unresolved conflict',
      () {
        final conflict = TimelineErrorMapper.unresolvedConflict(
          'Overlap detected',
        );
