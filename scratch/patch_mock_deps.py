...
# fix the remaining errors in editors (like string vs datetime assignment in fixed_schedule_editor)
with open('lib/features/onboarding/steps/base_timeline_editors/fixed_schedule_editor.dart', 'r') as f:
    content = f.read()
content = content.replace("updatedAt: DateTime.now().toIso8601String()", "updatedAt: DateTime.now()")
content = content.replace("touchUpdatedAt: true", "")
