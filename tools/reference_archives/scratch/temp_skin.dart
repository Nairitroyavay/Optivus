...
import 'package:optivus2/services/relative_timing_resolver.dart';
import 'package:optivus2/services/routine_acceptance_guard.dart';
import 'package:optivus2/services/routine_import_fallback_parser.dart';
...
class _SkinCareSetupScreenState extends ConsumerState<SkinCareSetupScreen> {
  ...
  void initState() {
    ...
    final templates =
        ref.read(routineProvider).routineTemplates['skin_care'] ?? const [];
  ...
  Future<void> _save(WidgetRef ref) async {
    ...
    try {
      ...
      debugPrint('[SkinCareSetup] accept_start');
      debugPrint('[SkinCareSetup] accept_templates_count=${templates.length}');
      ...
      debugPrint('[SkinCareSetup] accept_save_success');
      widget.onComplete();
    } on RoutineAcceptTimeoutException {
      debugPrint('[SkinCareSetup] accept_timeout');
      if (mounted) {
      ...
      debugPrint('[SkinCareSetup] accept_save_failed: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Could not save. Please try again.')),
        );
  ...
  Map<String, dynamic> _resolveTemplate(Map<String, dynamic> template) {
    return resolveRelativeTimingForTemplate(
      template: template,
      date: DateTime.now(),
      anchors: _anchorsFromRoutineState(ref.read(routineProvider)),
    );
  }

  List<RoutineAnchor> _anchorsFromRoutineState(RoutineState state) {
    final anchors = <RoutineAnchor>[];
    for (final template in state.fixedScheduleTemplates) {
      anchors.add(RoutineAnchor.fromMap(template.toMap()));
    }
    for (final entry in state.routineTemplates.entries) {
      for (final item in entry.value) {
        final map = Map<String, dynamic>.from(item);
        map['routineType'] = map['routineType'] ?? entry.key;
        anchors.add(RoutineAnchor.fromMap(map));
      }
    }
