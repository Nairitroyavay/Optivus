...
dynamic normalizeRoutineTimeOrNull(dynamic x, dynamic y) => null;
dynamic canonicalRelativeTimingRule(dynamic x, dynamic y) => null;
dynamic resolveRelativeTimingForTemplate(dynamic x, dynamic y) => null;
class RoutineReviewScreen extends StatelessWidget {
  ...
  const RoutineReviewScreen({Key? key, this.mode, this.originalBlocks, this.modifiedBlocks, this.originalTemplates, this.modifiedTemplates, this.onSave, this.onCancel}) : super(key: key);
