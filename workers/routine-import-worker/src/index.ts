...
function buildRoutineImportPrompt(
  ...
  const sourceRules = {
    ...
    work:
      "Extract every clearly timed work/business schedule item from the image: Office Work, Work, Shift, Client Calls, Meeting, meetings, Project Work, Team Sync, Training Session, Commute, Lunch Break / Break, Freelance Project, freelance/side-work, Business Hours. Preserve the visible title and time. Use hard_block for fixed timed blocks. Use category job. Do not ignore blocks just because they are not named exactly Work. Do not extract Gym/Exercise, Study/Reading, Online Course, Rest Day/No Work, or personal habit blocks as job candidates. For weekly grid images, days are columns and times are rows. Convert each visible timed cell into one candidate with repeatDays matching the day column. Treat all clearly timed work schedule items as fixed work/business blocks unless source text clearly says rest day/no work. Use flexible tasks only for to-dos without a visible time.",
    eating:
