...
/// Feature adapter for Work / Job schedule items.
class WorkTimelineAdapter implements TimelineFeatureAdapter<ClassRoutineBlock> {
  final Color accent;

  const WorkTimelineAdapter({this.accent = OptivusColors.brandAccent});
  ...
  /// Opens the shared edit sheet for a work routine block.
  static Future<bool?> showWorkEditSheet({
            ...
            return Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextFormField(
                    key: const Key('timeline-edit-job-title-field'),
                    controller: titleCtrl,
                    decoration: InputDecoration(
                      hintText: 'e.g. Shift / Part-Time Job',
                      filled: true,
                  ...
                  const SizedBox(height: 6),
                  TextFormField(
                    key: const Key('timeline-edit-workplace-field'),
                    controller: workplaceCtrl,
                    decoration: InputDecoration(
                      hintText: 'e.g. Office / Remote (optional)',
                      filled: true,
                      fillColor: Colors.white.withValues(alpha: 0.6),
                      border: OutlineInputBorder(
                        borderRadius: BorderRadius.circular(12),
                    ...
                    children: [
                      Expanded(
                        child: OutlinedButton(
                          key: const Key(
                            'timeline-edit-work-start-time-picker',
                          ),
                          onPressed: () async {
                      ...
                      Expanded(
                        child: OutlinedButton(
                          key: const Key('timeline-edit-work-end-time-picker'),
                          onPressed: () async {
                            final picked = await showTimePicker(
                              context: context,
                              initialTime: TimeOfDay(
