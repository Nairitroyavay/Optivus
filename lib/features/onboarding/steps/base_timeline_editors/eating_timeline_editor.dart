...
import 'package:optivus/features/onboarding/steps/base_timeline_editors/mock_dependencies.dart';
...
class EatingRoutineBlock {
  ...
  String? suggestionId;
  int? weekday;
  Map<String, dynamic>? sourceTemplate;
...
class _EatingSetupScreenState extends ConsumerState<EatingSetupScreen> {
  ...
  bool _hasInitializedFromLoadedRoutine = false;

  @override
  void initState() {
    super.initState();
    weeklyRoutines = _blankWeeklyRoutines();
    _loadExistingEatingRoutineFromProvider(ref.read(routineProvider));
  }

  void _loadExistingEatingRoutineFromProvider(RoutineState routineState) {
    if (_hasInitializedFromLoadedRoutine) return;
    if (!routineState.hasLoadedRoutine ||
        routineState.routineLoadError != null) {
      return;
    }

    final savedEatingTemplates =
        routineState.routineTemplates['eating'] ?? const [];
    if (savedEatingTemplates.isNotEmpty) {
      final savedBlocks = _blocksFromEatingTemplates(List<Map<String, dynamic>>.from(savedEatingTemplates));
      weeklyRoutines = {
        for (var day = 0; day < 7; day++)
          day: _withAddButtons(savedBlocks[day] ?? const [], day),
      };
      _colorIndex = savedEatingTemplates.length;
    } else {
      weeklyRoutines = _blankWeeklyRoutines();
    }
    _hasInitializedFromLoadedRoutine = true;
  }

  Map<int, List<EatingRoutineBlock>> _blankWeeklyRoutines() {
  ...
  Future<bool> _save(WidgetRef ref) async {
    ...
    try {
      ...
      debugPrint('[EatingSetup] accept_templates_count=${templates.length}');
      await runRoutineAcceptWithTimeout(
        () => notifier.setRoutineTemplates(
          'eating',
          templates,
          importMetadata: _pendingImportMetadata,
        ),
      );
      debugPrint('[EatingSetup] accept_save_success');
      widget.onComplete();
      return true;
    } on RoutineAcceptTimeoutException {
      debugPrint('[EatingSetup] accept_timeout');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
      ...
      debugPrint('[EatingSetup] accept_save_failed: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Could not save. Please try again.')),
        );
  ...
  Future<void> _showImportOptions() async {
    ...
    final workerReady = flags.routineImportWorkerReady;
        ...
        child: ListView(
          ...
          children: [
            ListTile(
              ...
              title: Text(
                photoReady
                    ...
                    : 'Mess Photo from Camera (Unavailable)',
              ),
              subtitle: photoReady
                  ? null
                  : const Text(
                      'Photo import needs image import, R2, and Worker configuration.',
                    ),
              enabled: photoReady,
            ...
            ListTile(
              ...
              title: Text(
                photoReady
                    ...
                    : 'Mess Photo from Gallery (Unavailable)',
              ),
              subtitle: photoReady
                  ? null
                  : const Text(
            ...
            ListTile(
              ...
              title: Text(
                workerReady
                    ? 'Generate Adaptive Meal Plan (Coming Soon)'
                    : 'Generate Adaptive Meal Plan (Unavailable)',
              ),
              subtitle: Text(
                workerReady
                    ? 'Adaptive AI eating is not available yet.'
                    : 'Adaptive AI unavailable because AI backend is not configured.',
              ),
              enabled: false,
            ),
          ],
  ...
  EatingRoutineBlock _mealBlockFromTemplate(Map<String, dynamic> template) {
    final sourceTemplate = RoutineTemplateModel.fromMap(
      template,
  ...
  String _itemsText(Map<String, dynamic> template) {
  ...
  String _menuItemsText(Map<String, dynamic> template) {
    final itemsText = _itemsText(template);
    if (itemsText.isNotEmpty) return itemsText;
  ...
  bool _canSave(RoutineState routineState) {
    return routineState.hasLoadedRoutine &&
        routineState.routineLoadError == null &&
        _hasInitializedFromLoadedRoutine &&
        !_isImportingPhoto &&
        !_isSaving;
  }

  Map<String, dynamic> _templateFromBlock(
    EatingRoutineBlock item,
    int day,
    bool isSensitive,
    DateTime now,
  ) {
    final original = item.sourceTemplate == null
        ? <String, dynamic>{}
        : Map<String, dynamic>.from(item.sourceTemplate!);
    final title = item.mealName.trim();
    final originalItemsText = _itemsText(original);
    final foodText = item.foodName.trim();
    final existingId = original['templateId']?.toString().trim() ?? '';
    final originalRoutineType =
    ...
    final template = <String, dynamic>{
      ...
      'isActive': original['isActive'] is bool ? original['isActive'] : true,
      'updatedAt': now.toIso8601String(),
    };

    if (original['createdAt'] == null) {
      template['createdAt'] = now.toIso8601String();
    }

    if (original['items'] is List) {
      if (foodText.isNotEmpty && foodText != originalItemsText) {
        template['items'] = foodText
            .split(',')
            .map((part) => part.trim())
            .where((part) => part.isNotEmpty)
            .toList(growable: false);
        template['notes'] = foodText;
      } else if (original['notes']?.toString().trim().isNotEmpty != true) {
        template.remove('notes');
      }
    } else {
      template['notes'] = foodText;
    }
    ...
    if (isSensitive) {
      template['sensitiveMode'] = true;
    }
    return template;
  }

  String _repeatRuleForBlock(
    EatingRoutineBlock item,
    int day,
    Map<String, dynamic> original,
  ) {
    final originalRule = original['repeatRule']?.toString().trim() ?? '';
    if (originalRule.isNotEmpty &&
        !originalRule.startsWith('mess_menu_weekday:')) {
      return originalRule;
    }
    final weekday = item.weekday ?? day + 1;
    return 'mess_menu_weekday:${_clampWeekday(weekday)}';
  }

  String _newEatingTemplateId(EatingRoutineBlock item, int day) {
    final titleSlug = item.mealName
        .trim()
        .toLowerCase()
        .replaceAll(RegExp(r'[^a-z0-9]+'), '_')
        .replaceAll(RegExp(r'^_+|_+$'), '');
    final timeSlug = _format24h(item.start).replaceAll(':', '');
    final safeTitle = titleSlug.isEmpty ? 'meal' : titleSlug;
    return 'eating_${day + 1}_${safeTitle}_$timeSlug';
  }
  ...
  Widget build(BuildContext context) {
    ...
    _loadExistingEatingRoutineFromProvider(routineState);
    final canSave = _canSave(routineState);
    const headerColors = [
      Color(0xFFFFD480),
      Color(0xFFFFA64D),
      Color(0xFFFF8080),
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: (_isImportingPhoto || !_hasInitializedFromLoadedRoutine)
            ? null
            : _showImportOptions,
